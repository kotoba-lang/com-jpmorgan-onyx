(ns jpmorgan-onyx.main-test
  "Contract + behavioral test for the jpmorgan-onyx L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "jpmorgan_onyx-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'jpmorgan-onyx.main-test)]
