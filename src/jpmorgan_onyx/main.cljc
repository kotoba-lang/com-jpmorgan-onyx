(ns jpmorgan-onyx.main
  "Kotodama WASM entrypoint for the JPMorgan Onyx clean-room actor (L4) — Clojure port.
  ...
  facts (`jpmorgan_onyx.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "jpmorgan_onyx")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"jpmorgan_onyx.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jpmorgan_onyx-compat" :tier tier :entities entities} 200])
...
(defn start! [] :jpmorgan_onyx-compat/ready)
